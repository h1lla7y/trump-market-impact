...
### 1. Clone & install
```bash
git clone https://github.com/hillary29jb/trump-market-impact.git
cd trump-market-impact
python -m venv .venv
...
# ANTHROPIC_API_KEY=your_key_here  # only needed for Claude classifier
```

### 3. Run the pipeline
```bash
# Fetch posts
python step1_fetch_posts.py

# Classify (choose backend: keyword | ollama | claude)
python step2_classify_posts.py --backend keyword

# Fetch market microstructure data (requires Databento key)
python step3_fetch_market_data.py

# Compute event study metrics
python step4_compute_abnormal_returnsv2.py

# Static charts → output/dashboard.png
python step5_visualise.py
